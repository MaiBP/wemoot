...
export function PublicRegistrationForm({
  ...
  const [audience, setAudience] = useState<"member" | "non_member">(
    "non_member",
  );
  ...
  const selectedPrice =
    availablePrices.find((item) => item.id === priceId) ?? availablePrices[0];
  const selectedProgram = programs.find((item) => item.id === programId);
  const requiresPayment =
    !advanced || selectedProgram?.payment_timing === "immediate";
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!destination)
        throw new Error("No recibimos el enlace para continuar.");
      window.location.assign(destination);
    } catch (cause) {
      setError(
        cause instanceof Error ? cause.message : "Ha ocurrido un error.",
      );
      setLoading(false);
    }
  }
  ...
  return (
    <form onSubmit={submit} className="space-y-6">
        ...
        <section className="space-y-4 rounded-2xl border border-brand-cyan/25 bg-brand-cyan/[.04] p-5">
          ...
          <div>
            ...
            <select
              ...
              className="h-11 w-full rounded-xl border border-brand-black/15 bg-white px-3 text-sm"
            >
          ...
          <div>
            ...
            <select
              ...
              value={audience}
              onChange={(event) => {
                setAudience(event.target.value as "member" | "non_member");
                setPriceId("");
              }}
              className="h-11 w-full rounded-xl border border-brand-black/15 bg-white px-3 text-sm"
            >
          ...
          <div>
            ...
            {!availablePrices.length && (
              <p className="mt-2 text-sm text-brand-magenta">
                No hay tarifas disponibles para esta selección.
              </p>
            )}
          </div>
            ...
            <div>
              ...
              >
                <option value="" disabled>
                  Selecciona una semana o periodo
                </option>
                {periods.map((period) => (
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-brand-black/50">
              El descuento y el total definitivo se validan en el servidor.
            </p>
          </div>
          <div className="flex items-center justify-between border-t border-brand-cyan/20 pt-4">
            <span className="text-sm text-brand-black/60">Total</span>
            <strong className="text-2xl">{formatCurrency(total)}</strong>
          </div>
          {selectedProgram?.payment_timing !== "immediate" && (
            <p className="rounded-xl bg-brand-yellow/30 p-3 text-sm">
              Esta modalidad reserva la plaza sin cobrar ahora
              {selectedProgram?.payment_due_date
                ? `; el pago se solicitará a partir del ${selectedProgram.payment_due_date}`
                : "."}
            </p>
          )}
        </section>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          <Label htmlFor="participant_email">Email de contacto</Label>
          <Input
        ...
        {advanced && (
          <>
            <div>
              <Label htmlFor="current_club">Club actual</Label>
              <Input id="current_club" name="current_club" />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              <Label htmlFor="medical_notes">Observaciones médicas</Label>
              <Textarea id="medical_notes" name="medical_notes" />
            </div>
          </>
        )}
        <div className="sm:col-span-2">
          <Label htmlFor="notes">Otras observaciones</Label>
          <Textarea id="notes" name="notes" />
        </div>
      </div>

      {advanced && (
        <label className="flex items-start gap-3 text-sm text-brand-black/65">
          <input type="checkbox" name="image_consent" className="mt-1" />
          <span>
            Autorizo el uso de imágenes del participante para la comunicación
            del evento.
          </span>
        </label>
      )}
      ...
      {requiresPayment && total > 0 && (
        <fieldset>
          <legend className="mb-2 text-sm font-medium text-brand-black/75">
            Forma de pago
          </legend>
          <div className="grid gap-3 sm:grid-cols-2">
            <label
              className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition ${method === "cash" ? "border-brand-cyan bg-brand-cyan/10 ring-2 ring-brand-cyan/15" : "border-brand-black/15"}`}
            >
              <input
                type="radio"
                value="cash"
              ...
              <span>
                <strong className="block text-sm">Efectivo</strong>
                <span className="text-xs text-brand-black/55">
                  Pagarás al organizador
                </span>
              </span>
            </label>
            <label
              className={`flex cursor-pointer items-center gap-3 rounded-xl border p-4 transition ${method === "card" ? "border-brand-magenta bg-brand-magenta/5 ring-2 ring-brand-magenta/10" : "border-brand-black/15"}`}
            >
              <input
                type="radio"
                value="card"
              ...
              <span>
                <strong className="block text-sm">Tarjeta</strong>
                <span className="text-xs text-brand-black/55">
      ...
      {error && (
        <p
          role="alert"
          className="rounded-xl bg-brand-magenta/10 p-3 text-sm text-brand-black"
        >
          {error}
        </p>
      )}
      <Button
        ...
        size="lg"
        className="w-full"
        disabled={loading || (advanced && !selectedPrice)}
      >
      ...
      <p className="text-center text-xs leading-5 text-brand-black/45">
        Al inscribirte, autorizas al organizador a usar estos datos para
